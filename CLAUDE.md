...
**Cost:** ~$16/month (Lambda $0 + RDS $12 + CloudFront $2 + Secrets $1.20)
**Updated:** 2025-11-07
...
npx sst secret set AdminEmail "info@paiss.me" --stage staging
npx sst secret set AdminPassword "your-admin-password" --stage staging
...
- Domain `filter-ical.de` must be verified in SES (eu-north-1)
- Sends from: `noreply@filter-ical.de`
- Admin notifications go to: `ADMIN_EMAIL` (info@paiss.me)

**Security:**
- Secrets stored in AWS Secrets Manager (encrypted at rest)
- SST automatically injects secrets into Lambda functions
- Frontend NEVER receives backend secrets (DB, JWT)
...
npx sst secret set AdminEmail "info@paiss.me" --stage production
npx sst secret set AdminPassword "your-admin-password" --stage production
```
