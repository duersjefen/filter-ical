            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                  ...
                  <span v-if="accountForm.email && !user?.email" class="text-red-600 dark:text-red-400">*</span>
                </label>
                <input
                  v-model="accountForm.newPassword"
                  type="password"
                  class="w-full px-4 py-2 border-2 border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg"
                  :placeholder="accountForm.email && !user?.email ? 'Required when adding email' : (user?.has_password ? 'Leave blank to keep current' : 'Optional: Add password for account security')"
                />
                <p v-if="accountForm.email && !user?.email && !accountForm.newPassword" class="text-xs text-red-600 dark:text-red-400 mt-1">
                  ⚠️ Password required when adding email (needed for password reset)
                </p>
                <p v-else-if="!user?.email && accountForm.newPassword" class="text-xs text-orange-600 dark:text-orange-400 mt-1">
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1">
              ...
              <p class="text-xs text-indigo-100">
                Domains you're actively using with filters, password access, or admin privileges
              </p>
            </div>
          </div>
        </div>
...
// Merge filter domains, password access domains, and admin domains
const activeDomains = computed(() => {
  ...
  adminDomains.value.forEach(domain => {
    const existing = domainMap.get(domain.domain_key)
    if (existing) {
      // Merge with existing, preserving admin access level
      domainMap.set(domain.domain_key, {
        ...existing,
        ...domain,
